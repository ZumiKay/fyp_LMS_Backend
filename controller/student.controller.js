...
const createuserSession = async (email , sessionid , user_id) => {
    
   
    try {
        await db.usersession.create({ user: email , sessionID: sessionid , user_id: user_id });
    } catch (error) {
        throw error;
    }
};
    ...
    Students.findOne({
        ...
        if (response) {
            ...
            if (isMatch) {
                ...
                await createuserSession(response.email  , sessionID , response.studentID);
                            ...
                            db.librarian
                                ...
                                .then(async (response) => {
                                    if (response) {
                                        ...
                                        if (isMatch) {
                                            const token = generateToken(response);
                                            await createuserSession(response.email , sessionID , response.cardID);
